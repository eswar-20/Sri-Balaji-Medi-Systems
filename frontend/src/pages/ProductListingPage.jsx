    ...
    const loadData = async () => {
      try {
        const [productsResp, categoriesResp] = await Promise.all([
          productAPI.getEquipmentProducts(),
          productAPI.getCategoriesByType('EQUIPMENT')
        ]);
        ...
        console.error('Error loading products from server, attempting local cache:', error);
        if (mounted) {
          try {
            const cachedProductsStr = localStorage.getItem('cached_equipment_products');
            const cachedCategoriesStr = localStorage.getItem('cached_equipment_categories');
            const validatedProducts = cachedProductsStr ? JSON.parse(cachedProductsStr) : [];
            const fetchedCategories = cachedCategoriesStr ? JSON.parse(cachedCategoriesStr) : ['All Categories'];
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="bg-[#142F52]/30 border border-white/[0.06] rounded-2xl p-6 sticky top-24 space-y-6 backdrop-blur-lg">
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <option value="500000-9999999">₹5,00,000+</option>
                </select>
              </div>
          ...
          <div className="lg:w-3/4 flex-grow">
            {loading ? (
              ...
              <EmptyState
                title="No medical equipment matching filters"
                description="We couldn't find any products matching your specific selection. Try resetting filters to browse all equipment."
                action={
                  <button
                    onClick={() => setFilters({
                      category: '',
